...
export class DC {
    ...
    @Route("/efkon/no/dc/ctrl/cmd/transactions/get/transactions/", HttpMethod.GET)
    getTransactions(req, res) {
        // const topic = io('http://localhost:65000/efkon/no/dc/ctrl/cmd/response/transactions');
        //
        ...
        //
        //
        // topic.on('connection', function(socket) {
        //     console.log("connect");
        //     socket.emit('transactions', transaction) ;
        // });
        // return topic.send(transaction);
    }

    @Route("/efkon/no/dc/ctrl/cmd/transactions/get", HttpMethod.GET)
    subscribeTopic(req, res) {
        let socket = io('ws://localhost:65000');
    ...
    @Socket('connection')
    onConnect(socket) {
        console.log(`Someone connected to DC.`);
    }
     ...
     * @param socket
     */
    @Socket('connection', '/transactions')
    onTopic(socket) {
        console.log('Someone subscribed to /transactions');
        ...
        socket.emit('transaction', {transaction: transaction});
    }
}
